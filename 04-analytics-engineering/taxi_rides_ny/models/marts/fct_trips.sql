/*
To Do:
- One row per trip (doesn't matter if it's yellow or green taxi)
...
select 
    ...
    -- Location details (enriched with human-redable zone names from dimension)
...
-- LEFT JOIN to preserve all trips even if zone info is missing (data quality issue)
...
{% if is_incremental() %}
    -- For incremental runs, only process new trips based on pickup_datetime
